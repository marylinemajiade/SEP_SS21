...
public class RMIServer implements RMIServerIF, Serializable {
    ...
    private final ObserverManagerI observerManager;
    //Attribute
...
/*        if(!lobby.getSpieler(0).contains(benutzername)) {
    ...
    @Override
    public ArrayList<Spielrunde> getAllSpielraueme() {
        ArrayList<Spielrunde> spielraueme = new ArrayList<>();
        spielrunden.values().forEach(i -> spielraueme.add(i.remoteSpielrunde));
        return spielraueme;
        //return new ArrayList<>(spielrunden.values());
    }
